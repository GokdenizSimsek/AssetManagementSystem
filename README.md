...
Projeyi kendi bilgisayarınızda derlemek ve çalıştırmak için aşağıdaki adımları izleyebilirsiniz. Geliştirme sürecinde GCC (MinGW) derleyicisi kullanılmıştır.

**1. Repoyu klonlayın:**
```bash
git clone [https://github.com/KULLANICI_ADIN/AssetManagementSystem.git](https://github.com/KULLANICI_ADIN/AssetManagementSystem.git)
cd AssetManagementSystem
```
**2. Kodu derleyin (g++ kullanarak):**
```bash
g++ *.cpp -o main
```
